...
namespace wincom.mobile.erp
{
	...
	public class EntryActivity : Activity,IEventListener
	{
		...
		private void butCancelClick(object sender,EventArgs e)
		{
			...
			using (var db = new SQLite.SQLiteConnection (pathToDatabase)) {
				var itemlist = db.Table<InvoiceDtls> ().Where (x => x.invno == invno&&x.CompCode==compCode&&x.BranchCode==branchCode);	
				double ttlamt= itemlist.Sum (x => x.netamount);
				double ttltax= itemlist.Sum (x => x.tax);
				var invlist =db.Table<Invoice> ().Where (x => x.invno == invno&&x.CompCode==compCode&&x.BranchCode==branchCode).ToList<Invoice> ();
				if (invlist.Count > 0) {
